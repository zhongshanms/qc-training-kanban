...
(function () {
      ...
      .map((b) => {
        ...
        if (b.imgs) {
          inner += b.imgs.map((img) =>
            '<figure style="margin:8px 0 16px;display:flex;gap:14px;align-items:flex-start">' +
            '<img src="' + img.src + '" alt="' + esc(img.alt) + '" loading="lazy" style="width:120px;height:120px;object-fit:cover;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,.1);flex-shrink:0">' +
            '<figcaption style="font-size:14px;color:var(--ink);line-height:1.6;padding-top:4px">' + esc(img.alt) + '</figcaption></figure>'
          ).join("");
        }
        if (b.table) {
  ...
  function homePage() {
    const cats = [
      ...
      { key: "light", ic: "💡", desc: "基础质检12项 · 欧美规区别 · 摔箱测试SOP" },
    ...
    const stats =
      ...
      '<div class="stat"><div class="n">12</div><div class="l">灯饰质检项</div></div>' +
